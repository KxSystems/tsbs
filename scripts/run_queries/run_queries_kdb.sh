#!/bin/bash
#
# TODO: This script does not work as yet. See manual instructions.
#

# Ensure runner is available

source ./scripts/q_sys/env     # to get QUERY port

EXE_FILE_NAME=${EXE_FILE_NAME:-$(which tsbs_run_queries_kdb)}
if [[ -z "$EXE_FILE_NAME" ]]; then
    echo "tsbs_run_queries_kdb not available. It is not specified explicitly and not found in \$PATH"
    exit 1
fi
...
NUM_WORKERS=${NUM_WORKERS:-$(grep -c ^processor /proc/cpuinfo 2> /dev/null || echo 1)}
...
if [ "$#" -gt 0 ]; then
    ...
    echo "Do not have any files specified - run from default queries folder as ${BULK_DATA_DIR}/queries_kdb*"
    for FULL_DATA_FILE_NAME in "${BULK_DATA_DIR}/queries_kdb"*; do
        run_file $FULL_DATA_FILE_NAME
    done
fi
